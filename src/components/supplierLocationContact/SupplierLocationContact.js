...
import { ContactListForSupplier } from "../../redux/reducers/contact/contact-actions";
...
class SupplierLocationContact extends React.Component {
  ...
  fn_contactList = async () => {  
    const ITEMS = await ContactListForSupplier(this.props.User.token);
  ...
  fn_CheckValidation = () => {
    ...
    if (this.state.cmbContactValue == null) {
      document.getElementById("errContact").innerHTML =
        "راه های ارتباطی را انتخاب نمائید";
      flag = false;
    }
          ...
          <Row className="standardPadding">
            <Row>
              <Label className="title">
                {" "}
                لیست تامین کنندگان و راه های ارتباط
              </Label>
            </Row>
            <Row>
