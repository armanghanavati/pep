...
class MainMenu extends React.Component {
  ...
  fn_Routing = async () => {
    ...
    const LINK_PATH = ORG_URL.substring(
      ORG_URL.indexOf(URL[3]),
      ORG_URL.length
    );
      ...
      for (let i = 0; i < tempPermission.length; i++)
        if (tempPermission[i].objectTreeTypeName == "MainMenu") {
          let obj = {
            ...
            LinkPath:
              tempPermission[i].linkName != null
                ? "/" + tempPermission[i].linkName
                : null,
            icon:
              tempPermission[i].icon != null &&
              window.siteAddress + tempPermission[i].icon,
          };
          ...
          <Col style={{ minWidth: "400px" }}>
            <div style={{ color: "black" }}>
              ...
              <Routes>
                <Route
                  exact
                  path={this.props.linkPath}
                  element={this.props.linkComponent}
                ></Route>
              </Routes>
            </div>
            <div style={{ color: "black" }}>
              ...
              <Routes>
                <Route
                  exact
                  path={this.state.linkPath}
                  element={this.state.linkComponent}
                ></Route>
              </Routes>
            </div>
          </Col>
