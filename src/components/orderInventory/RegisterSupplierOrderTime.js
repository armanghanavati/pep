...
class RegisterSupplierOrderTime extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      LocationList: null,
    ...
    if (perm != null)
      for (let i = 0; i < perm.length; i++) {
        switch (perm[i].objectName) {
          case "registerSupplierOrderTime.insert":
  ...
  btnShow_onClick = async () => {
    var result = await positionSupplierApproveOrderTime(
      ...
      <div className="standardMargin" style={{ direction: "rtl" }}>
        ...
        <Card className="shadow bg-white border pointer">
          <Row className="standardPadding">
            <Row>
              <Label>زمان بندی فروشگاه تامین کننده</Label>
            </Row>
            <Row className="standardPadding">
              <Col xs="auto">
...
export default connect(mapStateToProps)(RegisterSupplierOrderTime);
